...
Приложение поддерживает подключение к Telegram через MTProto Proxy. Параметры задаются в настройках, по умолчанию:

```text
Host:   127.0.0.1
Port:   1080
Secret: задаётся пользователем при первом запуске
```
